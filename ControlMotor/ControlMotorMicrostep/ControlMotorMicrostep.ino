...
Adafruit_StepperMotor *myMotor1 = AFMS.getStepper(200, 1); // Motor 1 conectado a M1 y M2
Adafruit_StepperMotor *myMotor2 = AFMS.getStepper(200, 2); // Motor 2 conectado a M3 y M4
...
void loop() 
{
  ...
  //  myMotor2->step(50, BACKWARD, MICROSTEP);
    ...
    {        
        ...
        } else 
        {
           // Solo lo movemos si el final de carrera no está a 1
            ...
            {  myMotor1->step(STEP, BACKWARD , MICROSTEP); }  
              posicionActual1 = posicionActual1 - SENTIDOMOTOR1*STEP;                         
        }
    ...
    {            
        ...
        {
            myMotor2->step(STEP, BACKWARD , MICROSTEP);           
            posicionActual2 = posicionActual2 - SENTIDOMOTOR2*STEP;           
        }
        // Actualizamos el error al final     
        error2 =  posicionRef2 - posicionActual2;    
...
void gotoReset1() {
    int valor = digitalRead(finCarrera1);
    // Lo sacamos de la posición de reset en caso de que estuviera
    if (valor==1)
    {
      // Lo desplazamos un poco para sacarlo de la posible posición de reset      
      // myMotor1->step(1000, BACKWARD  , MICROSTEP);                                                
    ...
    if (sign(SENTIDOMOTOR1)*sign(offset1)==1)
        myMotor1 -> step(abs(offset1), FORWARD , MICROSTEP); 
    else
        myMotor1 -> step(abs(offset1), BACKWARD   , MICROSTEP); 
...
//    // Lo sacamos de la posición de reset en caso de que estuviera
    ...
    {
      // Lo desplazamos un poco para sacarlo de la posible posición de reset
      // myMotor2->step(1000, BACKWARD  , MICROSTEP);                                                
